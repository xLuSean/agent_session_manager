...
最多保留 15 組本機環境的檢查結果。完整相容性驗證只在 Settings 明確執行；啟動、重新整理、回到前景及基本操作前，只比對已保存的安裝位置、Desktop 版本／build、CLI 與安裝檔案的輕量中繼資料，不重新計算大型執行檔指紋、驗證 App 簽章或掃描資料庫結構。相同安裝可跨啟動沿用；版本或安裝檔更換、位置改變、缺少通過紀錄時，提示到 Settings 檢查，不在背景自行驗證。即使版本號未變，檔案被替換仍要求確認；這是更新偵測，不是完整防竄改稽核。新增對話不會單獨讓結果失效。舊紀錄缺少輕量比對資訊，升級後需明確檢查一次。Desktop application 顯示 App 本身的版本及 build，與 bundled CLI 版本不同。

基本操作仍逐筆核對目標、保護條件及操作結果；Desktop 清理仍在實際清理前檢查來源結構、備份及指定範圍。批次中途偵測到安裝變動會停止，不自動重送。中斷批次若尚待相容性檢查，保留待恢復紀錄；環境核對通過後只回讀結果，確認已封存的原 Move to Trash 項目才補登 Trash，其餘保持不動。
...
本次永久刪除報告的 **Conversation space cleared** 顯示已清除對話檔案的邏輯大小。ASM 在整批操作前後各量一次檔案資訊，必要時以有限量的檔頭 metadata 確認歸屬，不載入整份對話；只有官方刪除成功、刪除前可量測且刪除後沒有剩餘匹配檔案的項目才計入。量測不完整會顯示涵蓋筆數，缺少數據不算成 0。這不是磁碟可用空間的差額，不計資料庫或備份；數字只供本次報告顯示，未寫入永久操作歷史。舊報告或中斷恢復後沒有完整前測資料，不會補估清除量。
